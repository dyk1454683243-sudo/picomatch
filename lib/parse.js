...
const parse = (input, options) => {
  ...
  const findMatchingClose = startIndex => {
    let depth = 1;
    let escaped = false;
    let quote = 0;
  ...
  const parenBodyNeedsGroup = closeIndex => {
    if (closeIndex === -1) {
      return false;
    }

    const body = input.slice(state.index + 1, closeIndex);
    let escaped = false;
    let quote = 0;
  ...
  const isLiteralParen = () => {
    if (state.parens > 0 || extglobs.length > 0) {
      return false;
    }

    // `@(...)` lands here: the `@` token is marked extglob and the `(` is next.
    if (prev && (prev.extglob === true || prev.type === 'at')) {
      return false;
    }

    // Regex non-capturing groups and lookarounds: `(?:`, `(?=`, `(?!`, `(?<`...
    if (peek() === '?' && /[!=<:]/.test(peek(2))) {
      return false;
    }

    const close = findMatchingClose(state.index);

    // Unmatched `(` must still increment `state.parens` so strictBrackets
    // can throw at the end of the parse, matching the previous behavior.
    if (close === -1) {
      return opts.strictBrackets !== true;
    }

    if (parenBodyNeedsGroup(close)) {
      return false;
    }

    // `(a)*` / `(a)+` are documented regex-style quantifiers after a group.
    // A following `?` stays a glob qmark, so `/foo/(a)?` does not match `/foo/a`.
  ...
  while (!eos()) {
     ...
     * Parentheses
     *
     * Bash treats bare `(` / `)` as literals. Picomatch still compiles them as
     * grouping when they form a real extglob (`?(`, `*(`, `+(`, `@(`, `!(`), a
     * regex extension (`(?:`, `(?=`, `(?!`, `(?<=`, `(?<!`), or a group whose
     * body needs grouping (`|`, wildcards, slashes, etc.). Inert wraps such as
     * `(a)` stay literal so `/foo/(a)` does not match `/foo/a` (issue #171).
     */
